...
public sealed class SkinsController : ControllerBase
{
    ...
    [HttpGet("search")]
    public async Task<IActionResult> Search([FromQuery] string? term) => Ok((await _catalogue.SearchLocalSkins(term ?? string.Empty)).Select(skin => new
    {
        id = skin.MarketHashName,
        text = skin.MarketHashName,
        skin = new { skin.Name, skin.Weapon, skin.Exterior, skin.MarketHashName, skin.Image }
    }));
    ...
    {
        if (skinId != skin.SkinId) return BadRequest(new ApiResponse(false, "Invalid skin update."));
