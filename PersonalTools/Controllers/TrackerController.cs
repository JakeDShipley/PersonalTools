...
public sealed class TrackerController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse>> Create([FromBody] TrackerItemRequest request)
    {
        try
        {
            string itemId = await _tracker.CreateItem(UserId, request.Type, request.Title, request.Description, request.Area, request.AssignedToUserId, request.ShowOnDashboard);
            return Ok(new ApiResponse(true, itemId));
        }
    ...
    [HttpPut("{itemId}")]
    public async Task<ActionResult<ApiResponse>> Update(string itemId, [FromBody] TrackerItemUpdateRequest request)
    {
        try
        {
    ...
    [HttpPut("{itemId}/move")]
    public async Task<ActionResult<ApiResponse>> Move(string itemId, [FromBody] TrackerItemMoveRequest request)
    {
        try
        {
    ...
    public async Task<ActionResult<ApiResponse>> SetStatus(string itemId, [FromBody] TrackerItemStatusRequest request)
    {
        try
        {
...
public sealed record TrackerItemRequest(string Type, string Title, string Description, string Area, string? AssignedToUserId, bool ShowOnDashboard);
public sealed record TrackerItemUpdateRequest(string Type, string Title, string Description, string Area, string Status, string? AssignedToUserId, bool ShowOnDashboard);
public sealed record TrackerItemMoveRequest(string Status, IReadOnlyList<string>? OrderedItemIds);
