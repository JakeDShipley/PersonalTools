...
namespace PersonalTools.Classes.Tracker
{
    public interface ITrackerFuncs
    {
        ...
        Task<string> CreateItem(Guid userId, string type, string title, string description, string area, string? assignedToUserId, bool showOnDashboard);
    ...
    public class TrackerFuncs : ITrackerFuncs
    {
        ...
        public async Task<string> CreateItem(Guid userId, string type, string title, string description, string area, string? assignedToUserId, bool showOnDashboard)
        {
        ...
        public Task MoveItem(string itemId, string status, IReadOnlyList<string> orderedItemIds)
        {
            ...
            if (string.IsNullOrWhiteSpace(itemId) || orderedItemIds.Count == 0 || orderedItemIds.Count > 500)
        ...
        {
            if (!Statuses.Contains(status)) throw new InvalidOperationException("That status isn't valid.");
            if (string.IsNullOrWhiteSpace(itemId)) throw new InvalidOperationException("That item isn't valid.");

            return _data.SetStatus(itemId, status);
        }

        public Task DeleteItem(string itemId) => _data.DeleteItem(itemId);
        ...
        // An empty string (from the modal's "No-one" option) means unassigned - normalise to null
        // here so the data layer only ever sees either a real GUID or null, never "".
        private static string? NormaliseAssignee(string? assignedToUserId) => string.IsNullOrWhiteSpace(assignedToUserId) ? null : assignedToUserId;
