...
public sealed class MatchesData : IMatchesData
{
    ...
    public Task<List<CSMatchObj>> GetMatches(long userId, string? profileId) =>
        _database.GetBulkDataSP("sp_cs_matches_get", Map, Parameters(("p_user_id", userId), ("p_profile_id", ProfileParam(profileId))));
    ...
    public async Task UpdateMatch(long userId, string matchId, CSMatchObj match) =>
